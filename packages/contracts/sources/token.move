module admin::my_token {
    use sui::{coin::{Self, Coin, TreasuryCap}, coin_registry as coin_registry};
    ...
    public fun burn(treasury_cap: &mut TreasuryCap<MY_TOKEN>, coin: Coin<MY_TOKEN>) {
        coin::burn(treasury_cap, coin);
    }

    /// Split a coin into two coins
    public fun split(coin: &mut Coin<MY_TOKEN>, amount: u64, ctx: &mut TxContext): Coin<MY_TOKEN> {
        coin::split(coin, amount, ctx)
    }

    /// Merge two coins into one
    public fun join(coin: &mut Coin<MY_TOKEN>, coin_to_merge: Coin<MY_TOKEN>) {
